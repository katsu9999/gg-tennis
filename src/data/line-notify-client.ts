...
export type { LineRoundPayload, LineRoundsPayload, LineSummaryPayload };
...
/** Same endpoint, whole-night payload (all rounds in one message). */
export async function sendLineRounds(payload: LineRoundsPayload): Promise<void> {
  const base = import.meta.env.VITE_SUPABASE_URL as string | undefined;
  if (!base) throw new Error("VITE_SUPABASE_URL is not set");
  const res = await fetch(`${base}/functions/v1/line-notify`, {
    method: "POST",
